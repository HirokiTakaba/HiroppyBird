 ...
 */
    ...
    public:
        ...
        {
            if (Member* member = FindMember(name)) return member->value;
            else
            {
        ...
        {
            RAPIDJSON_ASSERT(IsArray());
            if (data_.a.size >= data_.a.capacity) Reserve(data_.a.capacity == 0?kDefaultArrayCapacity:data_.a.capacity* 2, allocator);
        ...
        const GenericValue& Accept(Handler& handler) const
        {
            switch (GetType())
            {
                ...
                case kArrayType:
                    handler.StartArray();
                    for (GenericValue* v = data_.a.elements; v != data_.a.elements + data_.a.size; ++v) v->Accept(handler);
    ...
    private:
        ...
        Member* FindMember(const Ch* name)
        {
            ...
            for (Member* member = o.members; member != data_.o.members + data_.o.size; ++member)
                if (length == member->name.data_.s.length && memcmp(member->name.data_.s.str, name, length * sizeof(Ch)) == 0) return member;
 ...
 */
    ...
    public:
         ...
         */
        GenericDocument(Allocator* allocator=0, size_t stackCapacity=kDefaultStackCapacity): stack_(allocator, stackCapacity)
            , parseError_(0)
            , errorOffset_(0) {}
